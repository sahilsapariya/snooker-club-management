...
export interface CloseSessionInput {
  ...
  /**
   * Who is closing it. Sent for the caller's own clarity only - the function
   * takes the actor from the session, so this cannot be used to attribute a
...
/**
 * Ends a session and records what was taken, in one transaction.
 *
 * Through an RPC rather than an UPDATE, because closing is now two writes - the
 * session and its first payment - and two writes from a phone are two chances
 * to end up half-done. A session closed and marked paid with no money recorded
 * against it, or money recorded against a session that never closed, are both
 * unacceptable at a counter with a customer standing at it.
 *
 * Note what is not sent: `payment_status`, `paid_amount_minor` and `paid_at`.
 * Those are derived from the payments ledger by a trigger. Sending them would
 * let the session disagree with the money behind it. `actual_duration_seconds`
 * and `total_amount_minor` are generated columns and would be refused outright.
 */
export async function closeSession(input: CloseSessionInput): Promise<Session> {
  const result = await supabase.rpc('close_session', {
    p_session_id: input.sessionId,
    p_billable_seconds: input.charge.billableSeconds,
    p_table_charge_minor: input.charge.tableChargeMinor,
    p_discount_minor: input.discountMinor,
    p_frames_played: Math.max(0, Math.floor(input.framesPlayed)),
    p_payment_amount_minor: input.payment.paidAmountMinor,
    ...(input.payment.method === null ? {} : { p_payment_method: input.payment.method }),
